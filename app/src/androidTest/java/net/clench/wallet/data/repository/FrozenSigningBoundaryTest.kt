...
/** Public synthetic testnet keys/transactions only. No server, relay, or real funds. */
@RunWith(AndroidJUnit4::class)
class FrozenSigningBoundaryTest {
    @Test fun phoneSignerRejectsFrozenRetainedPsbtAndUnfreezeRestoresSigning() = runBlocking {
        WalletRepositoryFixture().use { f ->
            val phone = account("")
            val external = account("PUBLIC second cosigner")
            val wallet = f.repository.createMultisigWallet("PUBLIC phone-signing fixture", 2,
                listOf(phone.publicKey, external.publicKey),
                mapOf(0 to MultisigPhoneSignerSecret(WORDS, phone.secretKey)))
            assertTrue(f.settings.isOfflineMode())
            assertTrue(f.repository.hasMultisigPhoneSigner(wallet.id))
            val native = borrowedNative(f.repository, wallet.id)
            val funding = funding(native)
            funding.use {
                native.applyUnconfirmedTxs(listOf(UnconfirmedTx(funding, SEEN)))
                val address = f.repository.getLastAddress(wallet.id).address
                val draft = f.repository.createPsbt(wallet.id, address, 40_000L, 1f)
                assertEquals(0, partialSignatureCount(draft))
                val allowed = f.repository.signMultisigPsbtWithPhoneKeys(wallet.id, draft)
                assertEquals("Positive control must add exactly one actual phone signature", 1, partialSignatureCount(allowed))
                val outpoint = "${funding.computeTxid()}:0"
                assertEquals(listOf(outpoint), psbtInputs(draft))
                // Freeze only after the reviewable unsigned draft exists. Legacy aliases
                // exercise the same canonical metadata boundary as production coin control.
                f.database.utxoMetadataDao().upsert(UtxoMetadataEntity(
                    outpoint.substringBefore(':').uppercase() + ":0000", wallet.id, isFrozen = true))
                val rejection = runCatching {
                    f.repository.signMultisigPsbtWithPhoneKeys(wallet.id, draft)
                }.exceptionOrNull()
                assertTrue("Frozen retained input must fail before phone signing",
                    rejection is IllegalArgumentException && rejection.message?.contains("frozen") == true)
                assertEquals("Rejected attempt must not mutate original draft", 0, partialSignatureCount(draft))
                assertTrue(f.repository.hasMultisigPhoneSigner(wallet.id))
                f.database.utxoMetadataDao().upsert(UtxoMetadataEntity(
                    outpoint.substringBefore(':').uppercase() + ":0000", wallet.id, isFrozen = false))
                val recovered = f.repository.signMultisigPsbtWithPhoneKeys(wallet.id, draft)
                assertEquals(1, partialSignatureCount(recovered))
                assertEquals(psbtInputs(draft), psbtInputs(recovered))
            }
        }
    }

    @Test fun rejectedPassphraseFeeBumpLocksEphemeralStateAndRecoversAfterExplicitUnlockAndGraphReload() = runBlocking {
        WalletRepositoryFixture().use { f ->
            val passphrase = " \t "
            ...
            funding.use {
                ...
                Transaction(decodeHex(originalHex)).use { original ->
                    ...
                    native.applyUnconfirmedTxs(listOf(UnconfirmedTx(original, SEEN + 1uL)))
                    val originalId = original.computeTxid().toString()
                    val outpoint = "${funding.computeTxid()}:0"
                    assertEquals(listOf(outpoint), transactionInputs(original))
                    val row = TransactionEntity(originalId, wallet.id, 40_000L, 200L,
                        SEEN.toLong() * 1000L, 0, "SENT", destination)
                    f.database.transactionDao().insertAll(listOf(row))
                    ...
                    assertEquals(listOf(row), f.database.transactionDao().getForWallet(wallet.id))
                    ...
                    assertTrue(f.repository.isPassphraseWalletUnlocked(wallet.id))
                    // A fixture-only native graph reload models the successful resync result.
                    // This is not a network sync: offline mode stays enabled throughout.
                    revealFirstAddress(reloaded)
                    reloaded.applyUnconfirmedTxs(listOf(UnconfirmedTx(funding, SEEN), UnconfirmedTx(original, SEEN + 1uL)))
    ...
    /** Reflection is restricted to injecting public synthetic scan data into the real cached
     * native wallet; no mocks replace signing, selection, Room metadata, or recovery methods.
