...
Assessed source: v0.3.23 transport/privacy release candidate
...
| Complete setup, funding, read/verify, unseal, sweep, confirmation, and post-sweep inspection on a real card | PHYSICAL EVIDENCE REQUIRED | Execute every SATSCARD row in `physical-hardware-gates-v0.3.23.md` on the release candidate |
...
| Single-sig TAPSIGNER payment | MISSING | The imported wallet is watch-only in Clench v0.3.23 |
